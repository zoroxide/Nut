...
bool Engine::load_flat_terrain(const std::string &texturePath) {
  ...
  // Spawn coins randomly on the flat terrain so the player can collect them
  {
    float half = terrainSize_ * terrainScale_ * 0.5f;
    // keep a small margin from edges
    float margin = 2.0f;
    coins_.spawnRandomOnTerrain(20, -half + margin, half - margin, -half + margin, half - margin, 0.8f, terrain_);
  }
...
void Engine::mainloop() {
  ...
  glUniform3f(glGetUniformLocation(shaderProgram_, "lightDir"), -0.2f, -1.0f,
              -0.3f);
  glUniform3f(glGetUniformLocation(shaderProgram_, "lightColor"), 1.0f, 0.98f,
              0.9f);
  glUniform1i(glGetUniformLocation(shaderProgram_, "texture1"), 0);
  glUniform3f(glGetUniformLocation(shaderProgram_, "fogColor"), 0.53f, 0.8f,
              1.0f);
  glUniform1f(glGetUniformLocation(shaderProgram_, "fogDensity"), 0.008f);

  // sky shader texture unit binding (skybox cubemap will be bound to unit 1 at
  // render time)
  ...
  glUniform1i(glGetUniformLocation(skyShader_, "cloudEnabled"),
              cloudEnabled_ ? 1 : 0);
  ...
  while (!glfwWindowShouldClose(window_)) {
    ...
    // Draw sky
    glUseProgram(skyShader_);
    glUniformMatrix4fv(glGetUniformLocation(skyShader_, "invProj"), 1, GL_FALSE,
                       glm::value_ptr(invProj));

    // Provide full inverse view matrix; the vertex shader uses mat3(invView) so
    // translation is ignored and the sky remains fixed (no parallax from camera
    // position).
    glm::mat4 invView = glm::inverse(view);
    glUniformMatrix4fv(glGetUniformLocation(skyShader_, "invView"), 1, GL_FALSE,
                       glm::value_ptr(invView));
    glUniform1i(glGetUniformLocation(skyShader_, "hasSkybox"),
                sky_.hasCubemap() ? 1 : 0);

    // update animated uniforms
    // float t = (float)std::chrono::duration<double>(Clock::now() -
    // lastFrame_).count();
    ...
    glUniform1f(glGetUniformLocation(skyShader_, "cloudSpeed"), cloudSpeed_);
    glUniform1f(glGetUniformLocation(skyShader_, "cloudScale"), cloudScale_);
    glUniform1f(glGetUniformLocation(skyShader_, "cloudOpacity"),
                cloudOpacity_);

    // Bind skybox cubemap handled inside Skybox::draw

    // Draw full-screen triangle via Skybox class
    sky_.draw(invView, invProj, sky_.hasCubemap(), runTime, cloudEnabled_,
              cloudSpeed_, cloudScale_, cloudOpacity_);
  ...
  // --- Update coins and draw ---
  coins_.update(deltaTime_, cameraPos_);
  coins_.draw(shaderProgram_, view, proj);
  coinsCollected_ = coins_.getCollectedCount();
...
void Engine::updateMovement(float dt) {
  ...
  // sprinting
  float sp = moveSpeed_ * dt;
  if (keys_[GLFW_KEY_LEFT_SHIFT])
    sp *= SPRINT_MULTIPLIER; // sprint multiplier (ideal 1.9 for normal sprint)
  glm::vec3 move(0.0f);
  if (keys_[GLFW_KEY_W])
    move += front * sp;
  if (keys_[GLFW_KEY_S])
  ...
  float terrainY = terrain_.getHeightAt(cameraPos_.x, cameraPos_.z);
  float eyeHeight = 1.7f;
  if (jumping_) {
    cameraPos_.y += jumpVel_ * dt;
    jumpVel_ -= 18.0f * dt;
    if (cameraPos_.y <= terrainY + eyeHeight) {
